...
```bash
./nebula_pcap_visualizer project.json
```
...
Use `pcap_recorder_config.json` as the recorder config format:

```json
{
  "configuration_name": "all_sensors",
...
```bash
python3 scripts/record_pcaps.py pcap_recorder_config.json yard_test_01
```
...
After recording, generate a Nebula visualizer project from the experiment directory:

```bash
python3 scripts/visualize_recorded_pcaps.py pcap_recorder_config.json yard_test_01
```

This writes `recordings/yard_test_01/visualizer_project.json` using the recorded PCAPs that match the recorder naming convention.

The helper reads `model` from each sensor entry when present. For the provided `pcap_recorder_config.json`, the visualized sensors already use Nebula model names such as `EMX`, `E1`, `EM4`, `RobinW`, `RobinE1X`, `FTX180`, `FTX140`, `PandarAT128`, `HummingbirdD1`, and `FalconK`.

Sensors with `visualize: false` are skipped. Sensors are also skipped if their output PCAP file is missing.

To immediately run the backend visualizer after generating the project:

```bash
python3 scripts/visualize_recorded_pcaps.py \
  pcap_recorder_config.json \
  yard_test_01 \
  --run-binary /path/to/nebula_pcap_visualizer
```
